NMAmodel_binary=function(){    
  for(i in 1:ns){ ## loop through studies
    ...
    for(k in 1:na[i]){ ## loop through arms within each studya i 
    ...
    for(k in 2:na[i]){
      logit(p[i,t[i,k]]) <- mu[i] + delta[i,t[i,k]] ## model for linear predictor
  ...
  for(k in (ref+1):nt) {d[k] ~ dnorm(0,0.001)}
...
w.new[1] <- 0 
for (k in 2:nt) {
  yrep[k] ~ dnorm(m.new[k],tau.new[k])
